...
def main(args):
    ...
    logging.info(f'Device size {device_size}')
    logging.info(f'Device limit {device_limit}')
    logging.info(f'Device pool size {device_pool_size}')
...
def parse_args():
    ...
    job_name = 'test_{}'.format(time.strftime("%Y%m%d_%H%M%S"))

    args.out_path = os.path.join(args.out_path, job_name)
    profile_folder = os.path.join(args.profile, job_name)
    os.mkdir(profile_folder)
    args.profile = os.path.join(profile_folder, 'dask-profile.html')
